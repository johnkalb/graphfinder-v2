...
async def log_requests(request: Request, call_next):
    ...
    # and everything under the admin dashboard / debug tooling -- 2026-09-03:
    # a one-time purge found 70% of all logged events to that point were
    # Claude's own dev/admin-tool testing, not real visitors, precisely
    # because those surfaces were being logged like any other route. Excluded
    # here so the traffic numbers stay real-visitor-only going forward
    # instead of needing another purge later.
    path = request.url.path
    _is_dev_only_path = (
        path.startswith("/api/debug/") or path in ("/admin", "/analytics", "/api/analytics")
        or path.startswith("/api/service/")
    )
    if path not in ["/health", "/metrics", "/meminfo", "/debug", "/loadgraph"] and not _is_dev_only_path:
