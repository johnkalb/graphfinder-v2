...
@app.get("/api/debug/analytics-purge")
async def debug_analytics_purge(request: Request, confirm: str = ""):
    """TEMPORARY -- removes anonymous_events rows generated by Claude's own
    admin-tool testing today, not real visitors. A session_id is considered
    contaminated if it EVER touched a surface that's never linked from the
    real end-user UI (/api/debug/*, /admin, /analytics, /api/analytics,
    /api/service/*) -- that session's events are removed entirely (not just
    the dev-path ones), since the same browser session also generated
    incidental hits on ordinary paths (/, /api/search, /api/crawlie, ...)
    while testing. GET instead of POST/DELETE so it can be driven via plain
    navigation. Dry-run by default (no `confirm=yes`) -- returns counts and
    the session ids it would remove without deleting anything. Remove this
    endpoint once used."""
    ...
    conn = db.connect(db_path)
    c = conn.cursor()
    c.execute("SELECT id, session_id, metadata, timestamp FROM anonymous_events")
    rows = [dict(r) for r in c.fetchall()]

    def is_dev_path(p):
        p = p or ""
        return (p.startswith("/api/debug/") or p in ("/admin", "/analytics", "/api/analytics")
                or p.startswith("/api/service/"))
    ...
    for r in rows:
        try:
            ...
            meta = {}
        if is_dev_path(meta.get("path")):
            contaminated.add(r["session_id"])

    affected = [r for r in rows if r["session_id"] in contaminated]
    result = {
        "contaminated_sessions": sorted(contaminated),
        "events_before": len(rows),
        "unique_sessions_before": len(set(r["session_id"] for r in rows)),
        "events_to_remove": len(affected),
        "events_remaining_if_purged": len(rows) - len(affected),
        "purged": False,
    }
    if confirm == "yes" and affected:
        ids = [r["id"] for r in affected]
        placeholders = ",".join("?" for _ in ids)
        c.execute(f"DELETE FROM anonymous_events WHERE id IN ({placeholders})", ids)
        conn.commit()
        result["purged"] = True
    conn.close()
    return result
...
async function doResolve(id) {{
  const note = document.getElementById('note-' + id)?.value || null;
  const d = await postJson(`/api/service/items/${{id}}/resolve`, {{resolution_note: note}});
  if (d.success) setQueueStatus(`Item ${{id}} resolved.`);
  loadQueue();
}}
async function doNotify(id) {{
  const d = await postJson(`/api/service/notify/${{id}}`, {{}});
  if (d.success) setQueueStatus(`Notified for item ${{id}}.`);
  loadQueue();
}}
...
async function postJson(url, body) {{
  const btns = document.querySelectorAll('#queue-body button');
  btns.forEach(b => b.disabled = true);
  document.getElementById('queue-status').textContent = 'working...';
  try {{
    ...
    document.getElementById('queue-status').textContent = 'error: ' + e;
